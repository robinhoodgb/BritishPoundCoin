...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total britishpound balance of the wallet\n"
            "  \"obfuscation_balance\": xxxxxx, (numeric) the anonymized britishpound balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in britishpound/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in britishpound/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"britishpoundaddress\"\n"
            "\nReturn information about the given britishpound address.\n"
            "\nArguments:\n"
            "1. \"britishpoundaddress\"     (string, required) The britishpound address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"britishpoundaddress\", (string) The britishpound address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: britishpound address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are britishpound addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) britishpound address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"britishpoundaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"britishpoundaddress\"  (string, required) The britishpound address to use for the signature.\n"
