...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("britishpoundMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("britishpoundMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("britishpoundMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("britishpound-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running britishpoundMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
